                ...
                <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                  <div className="space-y-2">
                    ...
                    <p className="text-sm text-muted-foreground">
                      {[item.name, item.phone, item.email, [item.city, item.state].filter(Boolean).join(', ')]
                        .filter(Boolean)
                        .join(' | ')}
                    </p>
                    {item.subject ? <p className="text-sm font-medium">{item.subject}</p> : null}
                    {item.message ? <p className="max-w-4xl whitespace-pre-wrap text-sm leading-6 text-muted-foreground">{item.message}</p> : null}
